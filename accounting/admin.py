...
class JournalLineInline(UnfoldTabularInline):
    model = JournalLine
    extra = 2
...
class BankTransactionAdmin(UnfoldModelAdmin):
...
class TaxRuleAdmin(UnfoldModelAdmin):
...
class VATReturnAdmin(UnfoldModelAdmin):
