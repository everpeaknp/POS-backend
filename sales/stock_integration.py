...
from sales.accounting_integration import (
    post_sales_order_cogs,
    post_sales_order_revenue,
    reverse_sales_order_cogs,
    reverse_sales_order_revenue,
)
...
def handle_sales_order_status_change(sales_order, old_status, new_status, performed_by=None, warehouse_id=None):
    ...
    Cash sales also post revenue to GL on first fulfillment.
    """
    if new_status in ('Confirmed', 'Delivered') and old_status == 'Draft':
        ...
        post_sales_order_revenue(sales_order)
        return
    ...
    if new_status == 'Cancelled' and old_status in ('Confirmed', 'Delivered'):
        ...
        reverse_sales_order_revenue(sales_order)
